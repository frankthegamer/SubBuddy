    ...
    try:
        query = """
            SELECT U.USER_ID, U.USER_FName, U.USER_LName, S.SUB_ID, S.SUB_Name,
                   SP.SUBPAY_Cost, SP.SUBPAY_Date, SP.SUBPAY_Status, SV.SUBVER_FREQ
...
# --- ADMIN: LOOKUP USER + SUBSCRIPTIONS ---
...
def admin_get_user(request: Request, user_id: int):
    ...
    try:
        ...
        query = """
            SELECT S.SUB_ID, S.SUB_Name, S.SUB_SDate, S.SUB_CAT,
...
def admin_add_subscription(
    ...
    try:
        ...
        generate_payments(cursor, sub_id, sub_sdate, subver_cost, subver_freq, sub_status)
        conn.commit()
    except Exception as e:
        conn.rollback()
        print(f"Error: {e}")
